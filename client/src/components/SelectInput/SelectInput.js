...
class SelectInput extends React.Component {
    ...
    componentDidMount() {
      const {
        valueArray, optionsArray, input, meta: { dispatch, form, initial },
      } = this.props;
      if (!initial && optionsArray) dispatch(change(form, input.name, valueArray ? valueArray[0] : optionsArray[0]));
    }
